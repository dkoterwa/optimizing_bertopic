...
class Results:
    ...
    def visualize_table(self, dtm: bool = False, models: List[str] = None):
        ...
        return (
            ...
            .set_table_styles(
                {
                    (dataset, "npmi"): [{"selector": "td", "props": "border-left: 2px solid #000066"}]
                    for dataset in datasets
                },
                overwrite=False,
                axis=0,
    ...
    def visualize_table_tq(self, dtm: bool = False):
        ...
        for dataset in datasets:
            averaged_results[dataset] = to_plot[dataset]["npmi"] * to_plot[dataset]["diversity"]
            averaged_results[dataset] = averaged_results[dataset].astype(float).round(3)

        return (
            averaged_results.style.apply(highlight_max)
            .format(
                formatter={(dataset): "{:.3f}" for dataset in datasets for method in ["npmi", "diversity"]}
            )
            .set_properties(**{"width": "10em", "text-align": "center"})
            .set_table_styles([dict(selector="th", props=[("text-align", "center")])])
        )
    ...
    def _load_results(self):
        ...
        if "Dynamic Topic Modeling" in folders:
            for folder in os.listdir(self.main_folder + "Dynamic Topic Modeling"):
                self._load_dtm_results(f"{self.main_folder}Dynamic Topic Modeling/{folder}")
    ...
    def _load_all_results(self, dtm=False):
        ...
        if dtm:
            ...
            print(f"Items {self.basic_results.items()}")
            for dataset, dataframe in self.basic_results.items():
                print(f"Dataframe: {dataframe}")
                dataframe.to_csv("/Users/dominik/repos/bertopic_pooling_experiments/test123.csv")
                print("SAVED")
            data = {
                dataset: (
                    dataframe.groupby("Model")
        ...
        for dataset in data.keys():
            data[dataset] = data[dataset].set_index("Model").loc[order].reset_index().drop("Model", axis=1)

        # MultiIndex
        models.columns = pd.MultiIndex.from_product([[""], models.columns])

        for dataset in data.keys():
            data[dataset].columns = pd.MultiIndex.from_product([[dataset], data[dataset].columns])

        results = (
            pd.concat([models] + [data[dataset] for dataset in data.keys()], axis=1).round(3).astype(object)
        )
...
def highlight_max(data):
    ...
    if data.ndim == 1 and "BERTopic" not in data:  # Series from .apply(axis=0) or axis=1
        maximum = data.max()
