...
def evaluate_bbcnews_dataset(embeddings_path, save_path, number_of_runs):
    embeddings_data = np.load(embeddings_path, allow_pickle=True).item()
    dataset, custom = "./data/bbc_news", True
    for key in tqdm(embeddings_data.keys()):
        ...
        for i in range(number_of_runs):
            ...
            results = trainer.train(save=f"{save_path}BERTopic_bbcnews_{(key)}_{i+1}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="./embeddings_data/embeddings_bbc_news.npy",
    )
    parser.add_argument(
        "--results_save_path", type=str, help="Path to save our results", default="./results/bbcnews/"
    )
    parser.add_argument(
    ...
    evaluate_bbcnews_dataset(args.embeddings_path, args.results_save_path, args.n_of_runs)
